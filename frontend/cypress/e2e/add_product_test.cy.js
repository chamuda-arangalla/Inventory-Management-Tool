describe("Add Product Form", () => {
  ...
  it("fills the form and submits it successfully", () => {
    ...
    cy.get("input[placeholder='Enter product code']").type(uniqueCode);
  ...
  it("shows error on duplicate product code", () => {
    const duplicateCode = "TP-EXISTING";

    cy.get("input[placeholder='Enter product name']").type("Duplicate Product");
    cy.get("input[placeholder='Enter product code']").type(duplicateCode);
    cy.get("input[placeholder='Enter unit price']").type("100");
    cy.get("input[placeholder='Enter quantity']").type("5");
